...
class PerformanceAnalysis:
    ...
    def hoge_chart(self):
        def air_density(altitude, temp):
            ...
            temp_at_alt = T0 - lapse_rate * altitude + temp  # Adjusted temp at altitude
        ...
        for temp in temperature_range:
            for i in weight_values:
                mtow_v = i * self.g  # Update mtow with the current weight
                altitude = 0  # Reset altitude for each weight

                while True:
                    rho = air_density(altitude, temp)
                    v_i_mom = np.sqrt((mtow_v / 4) / (2 * rho * np.pi * self.rotor_radius ** 2))
                    ...
                    P_hoge_mom = P_i_mom + P_p_mom

                    power_left = P_available - P_hoge_mom

                    if power_left <= 0:
                        break

                    altitude += 10  # Increment altitude by 10 meters

                ceiling_altitudes_dict[temp].append(altitude)
    ...
    def hige_powers(self):
        ...
        def interpolate_ratio(z_D):
            keys = sorted(self.z_D_ratios.keys())
            values = [self.z_D_ratios[key] for key in keys]
            coefficients = np.polyfit(keys, values, 3)  # 3rd degree polynomial fit
        ...
        # Calculate P_hige for each height
        self.P_hige_values = self.P_p_hov + self.P_i_hov * self.ratio_values
    ...
    # use this function when you need power inputs for your analysis
...
class EnergyAnalysis:
    ...
    def visual_PEMFC_power(self):
        ...
        # calculating energy + power required from battery  -
        E_above =[]
        P_above = []
        for i in range(0,14):
