...
class BEMT():
    ...
    def discretise(self, num_elements=4):
        ...
        taper = 1 # -
        c_tip = c_root * taper
        self.c_r_list = c_root + ((c_tip - c_root) / self.R) * self.r_list

        # setup theta
        theta_root = np.deg2rad(20)
        theta_tip = np.deg2rad(5)
        self.theta_r_list = theta_root + ((theta_tip - theta_root) / self.R) * self.r_list
    ...
    def calculate_radius_and_rotors(self, num_elements=20):
        ...
        for chord, theta, a, r in zip(self.c_r_list, self.theta_r_list, self.a_r_list, self.r_list):
            prop = self.vertical(omega=self.omega, theta_r=theta, c_r=chord, r=r, dr=self.dr, a_r=a, V_c=1)
...
if __name__ == '__main__':
    ...
    n_rot = 2
    omega = 100
    thrust = 7900/np.cos(np.deg2rad(5))  # desired thrust in Newtons
